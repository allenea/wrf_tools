...
METHODS:
    - get_wps_info(filename,plot_info=False) -- Default
        -- Intialized when calling wps_info(filename)... Optional to print info right away

    - print_info()

    - get_proj_lambert_conformal()
        +(RETURNS: projection)
    ...
    - calc_center_point_latlon(corner_lat_parent, corner_lon_parent,\
                    dx_parent, dy_parent, e_we, e_ns, dx, dy, i, j, wpsproj, latlonproj) **
    ...
    - plot_all_domains(ax1, fig1, wpsproj, latlonproj, corner_lat_full,\
                           corner_lon_full, length_x, length_y)
        +(RETURNS: ax1, fig1 )
...
Code heavily adapted and corrected from https://github.com/lucas-uw/WRF-tools/blob/master/
            WRF_input_tools/Visualize_WPS_domain.ipynb and combined with my previous
attempts at reading/using/plotting wrf's namelist.wps file-data.


He had a great calc_wps_domain_info function.
    I modified it to work with my wps_info class replacing my reprojection code.
    - Decided not to implement the lambert projection gridlines work-around
        - Wait until cartopy implements it in a future version or
                let the user implement at their own peril

I was having trouble recalculating the centerpoint and then getting the corners
 perfect in all nested domains for all projection types.
Instead of just plotting the bounds of each domain, I have made it  so that you
 can create a cartopy projection and use only that domain,
 then from there you can choose to zoom in and out as you wish.

You do not have to use this class for plotting, but that is one of the features...
 The other main feature is it hold namelist.wps data
    - You can also use it to recreate your grid for other uses such as spatial
        interpolation or plotting observation data.
...
class wps_info:
    ...
    wpsproj, latlonproj, corner_lat_full, corner_lon_full,
            length_x, length_y = wps.calc_wps_domain_info()
    ...
    def get_wps_info(cls, namelist_wps_filename="namelist.wps"):
        ...
        for row in file:
            ...
            if len(row) != 0 and row[0] == "!":
                ...
                tmprow = row.split("=")[1] #All Start Dates for each domain should be the same
                tmprow = " ".join(tmprow.split("'"))
                ...
                tmprow = row.split("=")[1] #All End Dates for each domain should be the same
                tmprow = " ".join(tmprow.split("'"))
                tmprow2 = tmprow.split(" ")
    ...
    @classmethod
    def print_info(cls):
        """Print the output from the wps file"""
    ...
    @classmethod
    def get_proj_lambert_conformal(cls):
        """doc-string"""
        lccproj = ccrs.LambertConformal(central_longitude=cls.ref_lon,\
                            central_latitude=cls.ref_lat,\
                            standard_parallels=(cls.truelat1, cls.truelat2),\
    ...
    def calc_wps_domain_info(cls):
        ...
        if cls.map_proj == 'lambert':
            cls.wpsproj = wps_info.get_proj_lambert_conformal()
        ...
        corner_lon_full[0, 0], corner_lat_full[0, 0] =\
            wps_info.calc_corner_point_latlon(float(center_lat_full[0]), float(center_lon_full[0]),\
                                                               cls.e_we[0], cls.e_sn[0],\
                                                               float(cls.dx[0]), float(cls.dy[0]),\
                                                               cls.wpsproj, cls.latlonproj, 'll')
        corner_lon_full[0, 1], corner_lat_full[0, 1] =\
            wps_info.calc_corner_point_latlon(center_lat_full[0], center_lon_full[0],\
                                                   cls.e_we[0], cls.e_sn[0],\
                                                   cls.dx[0], cls.dy[0],\
                                                   cls.wpsproj, cls.latlonproj, 'lr')
        corner_lon_full[0, 2], corner_lat_full[0, 2] =\
            wps_info.calc_corner_point_latlon(center_lat_full[0], center_lon_full[0],\
                                                   cls.e_we[0], cls.e_sn[0],\
                                                   cls.dx[0], cls.dy[0],\
                                                   cls.wpsproj, cls.latlonproj, 'ul')
        corner_lon_full[0, 3], corner_lat_full[0, 3] =\
            wps_info.calc_corner_point_latlon(center_lat_full[0], center_lon_full[0],\
                                                   cls.e_we[0], cls.e_sn[0],\
                                                   cls.dx[0], cls.dy[0],\
        ...
        if cls.max_dom > 1:
            for i in np.arange(1, cls.max_dom):
                center_lon_full[i], center_lat_full[i] =\
                    wps_info.calc_center_point_latlon(corner_lat_full[i-1, 0],\
                                                      corner_lon_full[i-1, 0],\
                ...
                corner_lon_full[i, 0], corner_lat_full[i, 0] =\
                    wps_info.calc_corner_point_latlon(center_lat_full[i], center_lon_full[i],\
                                                           cls.e_we[i], cls.e_sn[i],\
                                                           cls.dx[i], cls.dy[i],\
                                                           cls.wpsproj, cls.latlonproj, 'll')
                corner_lon_full[i, 1], corner_lat_full[i, 1] =\
                    wps_info.calc_corner_point_latlon(center_lat_full[i], center_lon_full[i],\
                                                           cls.e_we[i], cls.e_sn[i],\
                                                           cls.dx[i], cls.dy[i],\
                                                           cls.wpsproj, cls.latlonproj, 'lr')
                corner_lon_full[i, 2], corner_lat_full[i, 2] =\
                    wps_info.calc_corner_point_latlon(center_lat_full[i], center_lon_full[i],\
                                                           cls.e_we[i], cls.e_sn[i],\
                                                           cls.dx[i], cls.dy[i],\
                                                           cls.wpsproj, cls.latlonproj, 'ul')
                corner_lon_full[i, 3], corner_lat_full[i, 3] =\
                    wps_info.calc_corner_point_latlon(center_lat_full[i], center_lon_full[i],\
                                                           cls.e_we[i], cls.e_sn[i],\
                                                           cls.dx[i], cls.dy[i],\
                                                           cls.wpsproj, cls.latlonproj, 'ur')
        ...
        return cls.wpsproj, cls.latlonproj, cls._corner_lat_full,\
                cls._corner_lon_full, cls._length_x, cls._length_y
    ...
    @staticmethod
    def calc_corner_point_latlon(center_lat, center_lon, e_we, e_ns, dx_h,\
                                     dy_v, wpsproj, latlonproj, loc):
        """ FROM Xiaodong Chen @lucas-uw on github"""

        center_x, center_y = wpsproj.transform_point(center_lon, center_lat, latlonproj)
        if loc == 'll':
            xpt = center_x - dx_h*e_we/2.0
            ypt = center_y - dy_v*e_ns/2.0
        elif loc == 'lr':
            xpt = center_x + dx_h*e_we/2.0
            ypt = center_y - dy_v*e_ns/2.0
        elif loc == 'ul':
    ...
    @staticmethod
    def calc_center_point_latlon(corner_lat_parent, corner_lon_parent, dx_parent, dy_parent,\
                                 e_we, e_ns, dx1, dy1, i, j, wpsproj, latlonproj):
        """ FROM Xiaodong Chen @lucas-uw on github"""
        corner_x_parent, corner_y_parent = wpsproj.transform_point(corner_lon_parent,\
                                                        corner_lat_parent, latlonproj)
        center_x_child = corner_x_parent + dx_parent*i + dx1*e_we/2.0
        center_y_child = corner_y_parent + dy_parent*j + dy1*e_ns/2.0
        center_lon_child, center_lat_child = latlonproj.transform_point(center_x_child,\
                                        center_y_child, wpsproj) # FROM #### to Lat/Lon
...
#%%   PLOT ZOOM(s) and PLOT_ALL_DOMAINS
    ...
    def plot_zoom_out(corner_x, corner_y, ns_zoom, we_zoom):
        ...
        if 0 <= ns_zoom < 40 and 0 <= we_zoom < 40:
            corner_x1 = corner_x + corner_x * we_zoom
            corner_y1 = corner_y + corner_y * ns_zoom
            return corner_x1, corner_y1
    ...
    def plot_zoom_in(corner_x, corner_y, ns_zoom, we_zoom):
        ...
        if 0 <= ns_zoom < 100 and 0 <= we_zoom < 100:
            corner_x1 = corner_x - corner_x * we_zoom/100.
            corner_y1 = corner_y - corner_y * ns_zoom/100.
            return corner_x1, corner_y1
        print("Invalid Zoom Level, No Zoom Applied")
        return corner_x, corner_y

    @staticmethod
    def plot_all_domains(ax1, fig1, max_dom, wpsproj, latlonproj, corner_lat_full,\
                         corner_lon_full, length_x, length_y):
        """
        Plot all the Domains

        Location of dNum string to be dynamic relative to domain size
        and figure size would be much better.
        ...
        colors = ['blue', "white", "red", "cyan", "magenta", "gold",\
        ...
        for i in range(max_dom):
            if i == 0:
                corner_x, corner_y = wps_info.reproject_corners(corner_lon_full[i, :],\
                                            corner_lat_full[i, :], wpsproj, latlonproj)
                ## In Geodetic Coordinates # Want Lat/Lon so I can add gridlines and labels on axis
            ...
            ax1.add_patch(mpl.patches.Rectangle((corner_x[0], corner_y[0]), length_x[i],\
                                length_y[i], fill=None, lw=3, edgecolor=colors[i], zorder=10))
        ...
        # I DO NOT LIKE THIS LAMBERT PROJ WORK AROUND.
        #   WAIT UNTIL CARTOPY SUPPORTS IT OR DON'T USE OR IMPLEMENT
...
#%%   Xiaodong Chen - Projection Features For Lambert Confromal Plots
    ...
    @staticmethod
    def find_side(lstring, side):
        """
        FROM Xiaodong Chen @lucas-uw on github
        Given a shapely LineString which is assumed to be rectangular, return the
        line corresponding to a given side of the rectangle.
        """
        minx, miny, maxx, maxy = lstring.bounds
    ...
    @staticmethod
    def lambert_xticks(axs, ticks, size):
        """FROM Xiaodong Chen @lucas-uw on github
             - Draw ticks on the bottom x-axis of a Lambert Conformal projection."""
        te_lambda = lambda xy: xy[0]
        lc_lambda = lambda t, n, b: np.vstack((np.zeros(n) + t, np.linspace(b[2], b[3], n))).T
        xticks, xticklabels = wps_info._lambert_ticks(axs, ticks, 'bottom', lc_lambda, te_lambda)
        axs.xaxis.tick_bottom()
        axs.set_xticks(xticks)
        axs.set_xticklabels([axs.xaxis.get_major_formatter()(xtick)\
                            for xtick in xticklabels], size=size)


    @staticmethod
    def lambert_yticks_left(axs, ticks, size):
        """FROM Xiaodong Chen @lucas-uw on github
            - Draw tricks on the left y-axis of a Lamber Conformal projection."""
        te_lambda = lambda xy: xy[1]
        lc_lambda = lambda t, n, b: np.vstack((np.linspace(b[0], b[1], n), np.zeros(n) + t)).T
        yticks, yticklabels = wps_info._lambert_ticks(axs, ticks, 'left', lc_lambda, te_lambda)
        axs.yaxis.tick_left()
        axs.set_yticks(yticks)
        axs.set_yticklabels([axs.yaxis.get_major_formatter()(ytick)\
                            for ytick in yticklabels], size=size)

    @staticmethod
    def lambert_yticks_right(axs, ticks, size):
        """FROM Xiaodong Chen @lucas-uw on github
            - Draw ricks on the left y-axis of a Lamber Conformal projection."""
        te_lambda = lambda xy: xy[1]
        lc_lambda = lambda t, n, b: np.vstack((np.linspace(b[0], b[1], n), np.zeros(n) + t)).T
        yticks, yticklabels = wps_info._lambert_ticks(axs, ticks, 'right', lc_lambda, te_lambda)
    ...
    @staticmethod
    def _lambert_ticks(axs, ticks, tick_location, line_constructor, tick_extractor):
        """FROM Xiaodong Chen @lucas-uw on github
            - Get the tick locations and labels for an axis of a Lambert Conformal projection."""
        outline_patch = sgeom.LineString(axs.outline_patch.get_path().vertices.tolist())
        axis = wps_info.find_side(outline_patch, tick_location)
        n_steps = 30
        extent = axs.get_extent(ccrs.PlateCarree())
        _ticks = []
        for tix in ticks:
            xy_line = line_constructor(tix, n_steps, extent)
            proj_xyz = axs.projection.transform_points(ccrs.Geodetic(), xy_line[:, 0],\
                                                       xy_line[:, 1])
            xyt = proj_xyz[..., :2]
            lstr = sgeom.LineString(xyt.tolist())
            locs = axis.intersection(lstr)
            if not locs:
                tick = [None]
            else:
                tick = tick_extractor(locs.xy_line)
            _ticks.append(tick[0])
        # Remove ticks that aren't visible:
        ticklabels = copy(ticks)
        #ticklabels.tolist()
        while True:
            ##Eric: HAD TO MAKE A CHANGE HERE BECAUSE OFTEN
                # ticklabels is a numpy array and not a list.
    ...
    #!! PROBLEM
    @staticmethod
    def set_lambert_ticks(axs, xskip=10., yskip=10., x_thickness=14, y_thickness=14):
        """
        PROBLEMS HERE.... extent_rounded doesn't match the real extent but if the
        extent is less than a skip increment then it becomes all messed up.
        labels are inconsistent. Not sure why it plots sometimes and not othertimes.
        I want to be able to have gridlines on set increments like at 5N,10N,15N,20N
        or 10N,20N,30N.
        Gridlines sometimes don't cover the entire area.
        """
        extent = list(axs.get_extent(ccrs.PlateCarree()))

        # Incase the extent is less than the x/y skip value.
            # But then that messes with the only working gridlines
        ...
        axs.xaxis.set_major_formatter(ticker.FormatStrFormatter("%3.2f"))
        axs.yaxis.set_major_formatter(ticker.FormatStrFormatter("%3.2f"))

        wps_info.lambert_xticks(axs, xticks, x_thickness)
        wps_info.lambert_yticks_left(axs, yticks, y_thickness)
        return axs
